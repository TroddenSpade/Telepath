...
class CycleVAE(nn.Module):
    ...
    def forward(self, args, agent, agent_2, a_1, a_2, observations, observations_2):
        ...
        reconst_observation = agent_2.observation_model(belief, posterior_state)
        ...
        return reconst_observation, mu_1
    ...
    def update_parameters(self, epochs, agent, agent_2, D, args, episode):
        batch_size = 1
        ...
        for i in tqdm(range(epochs)):
            observations, observations_2, a_1 = D.double_obs_sample(batch_size, 1)

            # observations = self.transformer(observations[0])
            # observations_2 = self.transformer(observations_2[0])
            observations = observations[0]
            observations_2 = observations_2[0]

            self.optimizer_G.zero_grad()
        
            reconst_observation, mu_1 = self.forward(args, agent, agent_2, a_1, a_1, observations, observations_2)

            loss_GAN_1 = lambda_0 * self.criterion_GAN(self.D1(reconst_observation), valid)
            ...
            loss_ID_1 = lambda_2 * self.criterion_pixel(observations_2[0], reconst_observation)
            ...
            loss_D1 = self.criterion_GAN(self.D1(observations_2), valid) + self.criterion_GAN(self.D1(reconst_observation.detach()), fake)
            loss_D1.backward()
            ...
            if i % 100 == 99:
                ...
                ax[1,1].imshow(observations_2[0].permute(1, 2, 0).detach().cpu().numpy()+0.5)
