...
def DTW(observations, reconstructed_observations):
    dim_1, dim_2 = observations.size(0), reconstructed_observations.size(0)
    shape = observations.shape
    observations = observations\
                    .unsqueeze(0).expand(dim_2, *shape)
    shape = reconstructed_observations.shape
    reconstructed_observations = reconstructed_observations\
                    .unsqueeze(1).expand(*shape[0:1], dim_1, *shape[1:])
    ...
    diff_matrix = torch.ones(obs_diffs.shape[0]+1, 
                             obs_diffs.shape[1]+1, 
                             obs_diffs.shape[2], 
    ...
    for i in range(obs_diffs.shape[0]):
        for j in range(obs_diffs.shape[1]):
